...
def interpolate_model(alpha):
    """
    If required, this function allows for interpolation between variations of the ESRGAN model.
    Currently, as we are only using one model, it simply adjusts the alpha blend with itself.
    """
    net_interp = OrderedDict()
    for k, v in model_ESRGAN.state_dict().items():
        net_interp[k] = v * alpha  # Blending ESRGAN model with itself for tuning (alpha scaling)

    model_interp = arch.RRDBNet(3, 3, 64, 23, gc=32)
    model_interp.load_state_dict(net_interp)
    model_interp.eval()
